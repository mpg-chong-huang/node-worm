...
superagent
   ...
   .end(function(err, res){      	
		...
		async.mapLimit(heroes, 1, 
			...
			function (err, result) {
				console.log('heros number：' + heroes.length);
			}
		);
    }); 
...
var gifURL = 'http://wcatproject.com/img/as2/';
var imgURL = 'http://wcatproject.com/img/img2d/';

var fetchInfo = function(heroId, callback){

	var url = imgURL+heroId+'.png';

	var filepath = 'img/'+heroId+'.png';
	...
	fs.exists(filepath, function(exists) {
        if (exists) {
            ...
            // 文件不存在，开始下载文件
            concurrencyCount++;
            console.log('loading：', concurrencyCount, '，load image id', url);
	...
	concurrencyCount++;
	 ...
	 console.log("now get the id "+ heroId + " + with" + concurrencyCount);
    // 根据角色ID，进行详细页面的爬取和解析
	superagent
